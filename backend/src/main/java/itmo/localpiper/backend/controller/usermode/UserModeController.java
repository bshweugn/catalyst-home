...
import itmo.localpiper.backend.service.processing.commands.CameraCommandProcessorService;
...
public class UserModeController {
    ...
    @Autowired
    private CameraCommandProcessorService cameraCommandProcessorService;
    ...
    public ResponseEntity<HoldableResultResponse<Camera>> executeCameraCommand(@Valid @RequestBody DeviceCommandRequest request, HttpServletRequest servletRequest) {
        return ResponseEntity.ok(cameraCommandProcessorService.process(requestTransformer.transform(request, servletRequest)));
    }
